...
const PLAY_LIST_DIRECTORY = DIRECTORIES.PLAY;
...
export const processTemplates = (data, play_path) => {
  processTemplate(data, 'common', play_path);
  processTemplate(data, 'package', play_path);
...
const replaceTemplateVar = (play_data, filePath) => {
  fs.readFile(filePath, 'utf8', function (err, data) {
    ...
    Object.keys(play_data).forEach((key) => {
      updated_result = replaceAll(updated_result, `%PLAY_${key.toUpperCase()}%`, play_data[key]);
    });
...
const getTargetFileName = (source_file_name) => {
  ...
  const file_name_wo_ext = source_file_name.substring(0, source_file_name_ext_index); // 'component_js'

  const source_file_name_segment = file_name_wo_ext.split('_'); // ['component', 'js']
  const new_file_ext = source_file_name_segment[source_file_name_segment.length - 1]; // 'js'
...
export const processTemplate = (play, template_type, play_path) => {
  TEMPLATE_MAP[template_type].forEach(async (file) => {
    ...
    const target_path = `${play_path}/${getTargetFileName(target_template_name)}`;
    await fs.copyFile(
      file_path,
      target_path,
      (res) => {
