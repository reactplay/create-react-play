...
const writeFile = (plays, dirs) => {
  ...
  plays.forEach(async (play, index) => {
    ...
    if (dirs.indexOf(playPath) > -1) {
      ...
      if (!hasPublicThumbImage && play?.cover) {
      ...
      cleanUpFileExtension(DIRECTORIES.PLAY, playPath);
      ...
      if (!fs.existsSync(`${DIRECTORIES.PLAY}/${playPath}/package.json`)) {
        processTemplate(play, 'package', `${DIRECTORIES.PLAY}/${playPath}`);
      }
  ...
  !hasPublicThumbImage && createPublicImages();
